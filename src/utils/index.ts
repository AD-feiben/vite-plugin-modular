  ...
  if (existsSync(templateDirPath)) {
    try {
      ...
      const entryFiles = templateFiles.filter((file: string) => {
        ...
        return ['index', 'main', 'app', 'entry', 'bootstrap', 'init', 'start'].includes(fileName)
      })
...
// 复制模板目录中的文件到模块目录
export function copyTemplateFiles(templateDir?: string, sourceDir?: string): string[] {
  const dir = templateDir || 'templates'
  const templateDirPath = join(process.cwd(), dir)
...
`
  writeFileSync(configPath, warningComment + JSON.stringify(config, null, jsonIndent), 'utf8')
}
...
export function createEnvFile(moduleName: string, envName: string): void {
  ...
  writeFileSync(
    envPath,
    '# Environment variables for module: ' + moduleName + ', env: ' + envName + '\n',
    'utf8'
  )
}
