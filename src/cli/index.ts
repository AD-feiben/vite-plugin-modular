...
function printBanner() {
  ...
  const description = ' - 🚀 Modularize Your Vite App ' + chalk.green(`v${VERSION}`)
...
const shouldPrintBanner =
  process.argv.length === 2 || process.argv[2] === '--help' || process.argv[2] === '-h'
...
program.name('vmod').description('Vite Plugin Modular CLI Tool').version(VERSION)

// 添加模块命令
program.command('add').alias('a').description('Add a new module').action(addCommand)
...
// 列表查询命令
program.command('list').alias('ls').description('List all modules').action(listCommand)
