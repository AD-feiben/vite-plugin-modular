...
export async function addCommand(): Promise<void> {
  try {
    ...
    // 添加命令到package.json
    packageJson.scripts[`serve:${moduleName}`] = `vite --mode ${moduleName}:dev`
    packageJson.scripts[`build:${moduleName}`] = `vite build --mode ${moduleName}:prod`
