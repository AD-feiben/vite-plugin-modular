...
export default function VitePluginModular(): Plugin {
  ...
  return {
    ...
    config(config, env) {
      ...
      if (!currentModule) {
        console.warn(`Module "${moduleName}" not found in .modular.config.jsonc`)
        return config
      }
      ...
      if (currentModule.define) {
        for (const [key, value] of Object.entries(currentModule.define)) {
          ...
          updatedConfig.define[`import.meta.env.VITE_${snakeCaseKey}`] = JSON.stringify(value)
        }
      }
    ...
    transformIndexHtml(html) {
      ...
      const normalizedBase = processedBase.endsWith('/') ? processedBase : `${processedBase}/`
      const fullEntryPath = getFullEntryPath(currentModule.sourceDir, currentModule.entry)
